...
let renderer;let scene;let camera;function setRendererObj(arg_renderer,arg_scene,arg_camera){renderer=arg_renderer;scene=arg_scene;camera=arg_camera}
...
function Link(){}
Link.prototype={isNewTab:'OFF',isShineOnMouse:'OFF',isShineOnTouch:'ON',shineColor:0x888888,emissiveObject:undefined,setNewTab:function(value,is_all=undefined){if(is_all==='ALL')
...
parent.removeChild(el);const area=1;el=document.createElement('a');parent.appendChild(el,null);el.id='Anchor3Dink';el.setAttribute('href',this.itsModel.link.url);el.style.display='inline-block';el.style.top=e.touches[0].clientY-this.rect.top-area/2+'px';el.style.left=e.touches[0].clientX-this.rect.left-area/2+'px';el.style.width=area+'px';el.style.height=area+'px';el.style.position='absolute';parent.style.position='relative'},shineModelFn:function(){let selectedMatl=this.selectedModel.material;const style=renderer.domElement.style;if(this.itsModel){if(this.itsModel.link||this.itsModel.parent.link){this.setParentObj();if(this.itsModel.link.isShineOnMouse!=='OFF'&&this.itsModel.link.url){if(!this.itsModel.material.emissive.r&&!this.itsModel.material.emissive.g&&!this.itsModel.material.emissive.b)
this.setShineModel(this.itsModel);style.cursor=this.cursorOn3Dink}}
else style.cursor=this.cursorDefault;if(this.selectedModel!==this.itsModel){this.resetShineModel(selectedMatl);this.selectedModel=this.itsModel}}
else if(style.cursor!==this.cursorDefault){style.cursor=this.cursorDefault;this.resetShineModel(selectedMatl)}},changeCursorFn:function(){const style=renderer.domElement.style;if(this.itsModel){if(this.itsModel.link||this.itsModel.parent.link){this.setParentObj();if(this.itsModel.link.url){style.cursor=this.cursorOn3Dink}}
else style.cursor=this.cursorDefault}
else if(style.cursor!==this.cursorDefault){style.cursor=this.cursorDefault}},shineModelFnTouch:function(){let selectedMatl=this.selectedModel.material;if(this.itsModel){if(this.itsModel.link||this.itsModel.parent.link){this.setParentObj();if(this.itsModel.link.isShineOnTouch!=='OFF'&&this.itsModel.link.url){if(!this.itsModel.material.emissive.r&&!this.itsModel.material.emissive.g&&!this.itsModel.material.emissive.b)
...
else if(selectedMatl.emissive.r&&selectedMatl.emissive.g&&selectedMatl.emissive.b){this.resetShineModel(selectedMatl)}},shineModelA:function(e){let selectedMatl=this.selectedModel.material;const style=renderer.domElement.style;const el=global.document.getElementById('Anchor3Dink');const parent=renderer.domElement.parentNode;if(this.itsModel){if(this.itsModel.link||this.itsModel.parent.link){this.setParentObj();if(this.itsModel.link.isShineOnMouse!=='OFF'&&this.itsModel.link.url){if(!this.itsModel.material.emissive.r&&!this.itsModel.material.emissive.g&&!this.itsModel.material.emissive.b)
this.setShineModel(this.itsModel);style.cursor=this.cursorOn3Dink;this.addAnchorMouse(e,el,parent)}
else{style.cursor=this.cursorDefault;if(el!==null)
parent.removeChild(el)}
if(this.selectedModel!==this.itsModel){this.resetShineModel(selectedMatl);this.selectedModel=this.itsModel}}
else if(el!==null)
parent.removeChild(el)}
else　if(style.cursor!==this.cursorDefault){style.cursor=this.cursorDefault;this.resetShineModel(selectedMatl)}
else if(el!==null)
parent.removeChild(el)},changeCursorA:function(e){const style=renderer.domElement.style;const el=global.document.getElementById('Anchor3Dink');const parent=renderer.domElement.parentNode;if(this.itsModel){if(this.itsModel.link||this.itsModel.parent.link){this.setParentObj();if(this.itsModel.link.url){style.cursor=this.cursorOn3Dink;this.addAnchorMouse(e,el,parent)}
else{style.cursor=this.cursorDefault;if(el!==null)
parent.removeChild(el)}}}
else if(style.cursor!==this.cursorDefault){style.cursor=this.cursorDefault;if(el!==null)
parent.removeChild(el)}},shineModelATouch:function(e){let selectedMatl=this.selectedModel.material;const el=global.document.getElementById('Anchor3Dink');const parent=renderer.domElement.parentNode;if(this.itsModel){if(this.itsModel.link||this.itsModel.parent.link){this.setParentObj();if(this.itsModel.link.isShineOnTouch!=='OFF'&&this.itsModel.link.url){if(!this.itsModel.material.emissive.r&&!this.itsModel.material.emissive.g&&!this.itsModel.material.emissive.b)
...
parent.removeChild(el)}},addAnchorItsObjA:function(e){const el=global.document.getElementById('Anchor3Dink');const parent=renderer.domElement.parentNode;if(this.itsModel){if(this.itsModel.link||this.itsModel.parent.link){this.setParentObj();if(this.itsModel.link.url){this.addAnchorTouch(e,el,parent)}
else{if(el!==null)
parent.removeChild(el)}}}
else if(el!==null)
parent.removeChild(el)},makeEventFnc:function(getPoint,process){return function(e){this.rect=renderer.domElement.getBoundingClientRect();const pointer=getPoint(e);const intersects=this.getIntersectObj(pointer,camera,scene);if(intersects[0])
...
else if(e.button===1)global.open(this.itsModel.link.url);else if(e.button===2)console.log(this.itsModel.link.url)}}}},addFnc:function(is_Hyperlink_mode='Fn',is_Hyperlink_mode_touch='Fn'){if(!WIDTH){WIDTH=renderer.domElement.style.width;WIDTH=WIDTH.substr(0,WIDTH.length-2)}
...
console.log('Touch: Anchor Tag Mode by 3Dink.js')}},};function createBox(g_x,g_y,g_z,p_x,p_y,p_z,txr=undefined,arg_color=undefined){const geometry=new webGlLib.BoxGeometry(g_x,g_y,g_z);if(arg_color!==undefined)
var material=new webGlLib.MeshPhongMaterial({color:arg_color});if(txr!==undefined){const texture=new webGlLib.TextureLoader().load(txr);texture.minFilter=webGlLib.LinearFilter;var material=new webGlLib.MeshPhongMaterial({map:texture})}
const model=new webGlLib.Mesh(geometry,material);model.position.set(p_x,p_y,p_z);return model}
exports.setWrapperLib=setWrapperLib;exports.setCanvasSize=setCanvasSize;exports.setRendererObj=setRendererObj;exports.addURL=addURL;exports.domEvent=domEvent;exports.createBox=createBox})))